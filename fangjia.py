...
test_search_dict = {'昌平': {'霍营': {'13号线': 'http://cd.fangjia.com/ershoufang/--r-%E6%98%8C%E5%B9%B3|w-13%E5%8F%B7%E7%BA%BF|b-%E9%9C%8D%E8%90%A5'}}}
...
def get_info(fin_search_list, process_i):
    print u'进程%d开始' % process_i
    fin_info_list = []
    for i in range(len(fin_search_list)):
        url = fin_search_list[i][-1]
        page = download(url)
        if not page:
            continue
        soup = BS(page, 'lxml')
        ...
        for num in range(20):
            ...
            try:
                ...
                tag_tmp_list = copy.deepcopy(fin_search_list[i][:-1])
...
def save_excel(fin_info_list, file_name):
    ...
    book = xlsxwriter.Workbook(file_name)  # 默认存储在桌面上
...
if __name__ == '__main__':
    ...
    '''    
    p = Pool(4)  # 设置进程池
    assignment_list = assignment_search_list(fin_info_list, 2)  # 分配任务，用于多进程
    result = []  # 多进程结果列表
    ...
    p = Pool(4)
    result = []
    urls = [url[-1] for url in fin_info_list]
